...
def imagerInit(server, imgDev):
	...
	if (icType[0]==2):
		numberOfRows		= 240
		numberOfColumns	 = 320
		numberOf3DimageDataframe = 4	 # 1 Distance + 1 Amplitued = 2 images
	elif (icType[0]==4):
		numberOfRows		= 60
		numberOfColumns	 = 160
		numberOf3DimageDataframe = 4	 # 1 Distance + 1 Amplitued = 2 images
	...
	if setModulation:							  	#set mod frequency and integration times if needed
		...
		server.sendCommand('setIntegrationTime2D 3500')	 # t_int in us
		server.sendCommand('setIntegrationTime3D 3500')	  # t_int in us
